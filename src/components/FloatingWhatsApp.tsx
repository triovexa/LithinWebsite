import { useState, useEffect } from 'react';
import { ChevronUp } from 'lucide-react';

export default function FloatingWhatsApp() {
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [typedText, setTypedText] = useState('');
  const [showBubble, setShowBubble] = useState(true);

  const fullMessage = "Hi, I am Arul! Any doubt? Ask me";
  const whatsappNumber = "919566738884";
  const defaultMessage = encodeURIComponent("Hi Lithin Transport, I would like to book container transport services.");
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${defaultMessage}`;

  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 200) {
        setShowScrollTop(true);
      } else {
        setShowScrollTop(false);
      }
    };

    window.addEventListener('scroll', handleScroll, { passive: true });
    handleScroll();

    return () => {
      window.removeEventListener('scroll', handleScroll);
    };
  }, []);

  // Recurring Typewriter cycle: Type out -> Stay 3s -> Hide 3s -> Repeat
  useEffect(() => {
    let typingInterval: ReturnType<typeof setInterval>;
    let timer: ReturnType<typeof setTimeout>;
    let isCancelled = false;

    const runTypewriterCycle = () => {
      if (isCancelled) return;

      let index = 0;
      setTypedText('');
      setShowBubble(true);

      typingInterval = setInterval(() => {
        if (isCancelled) return;
        index++;
        setTypedText(fullMessage.slice(0, index));

        if (index >= fullMessage.length) {
          clearInterval(typingInterval);

          // Wait 3 seconds with full message displayed
          timer = setTimeout(() => {
            if (isCancelled) return;
            setShowBubble(false);

            // Wait 3 seconds while hidden, then restart cycle
            timer = setTimeout(() => {
              if (isCancelled) return;
              runTypewriterCycle();
            }, 3000);
          }, 3000);
        }
      }, 45);
    };

    runTypewriterCycle();

    return () => {
      isCancelled = true;
      if (typingInterval) clearInterval(typingInterval);
      if (timer) clearTimeout(timer);
    };
  }, []);

  const scrollToTop = () => {
    window.scrollTo({
      top: 0,
      behavior: 'smooth'
    });
  };

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 pointer-events-none">
      {/* Go to Top Button - Bold ^ Symbol Only */}
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          aria-label="Scroll to top of page"
          title="Go to Top"
          className="pointer-events-auto w-10 h-10 flex items-center justify-center bg-[#060b1e]/90 hover:bg-emerald-500/20 backdrop-blur-xl border border-emerald-400/50 text-emerald-400 rounded-full shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:scale-110 transition-all duration-300 group cursor-pointer animate-bounce"
        >
          <ChevronUp className="w-5 h-5 text-emerald-400 stroke-[3] group-hover:-translate-y-0.5 transition-transform" />
        </button>
      )}

      {/* Typewriter Speech Bubble - disappears 3s after typing completes */}
      {showBubble && typedText && (
        <div className="pointer-events-auto relative max-w-[220px] bg-[#060b1e]/95 backdrop-blur-xl border border-emerald-400/50 text-white font-semibold text-[11px] leading-tight px-3.5 py-2 rounded-2xl rounded-br-none shadow-[0_0_20px_rgba(16,185,129,0.3)] flex items-center gap-2 transition-all duration-500 animate-in fade-in">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping shrink-0" />
          <span>{typedText}</span>
          {typedText.length < fullMessage.length && (
            <span className="w-1 h-3 bg-emerald-400 animate-pulse ml-0.5 inline-block shrink-0" />
          )}
        </div>
      )}

      {/* Dark Neon Cyberpunk WhatsApp Action Button with Bounce Jump & Glassy Dark Finish */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat with Arul on WhatsApp"
        className="pointer-events-auto relative w-14 h-14 flex items-center justify-center bg-slate-950/50 hover:bg-slate-900/80 backdrop-blur-md border-2 border-emerald-400/90 rounded-full shadow-[0_0_25px_rgba(16,185,129,0.5)] hover:shadow-[0_0_35px_rgba(56,189,248,0.7)] hover:scale-110 transition-all duration-300 group cursor-pointer animate-bounce"
        title="Chat with Arul on WhatsApp"
      >
        {/* Animated Rotating Emerald-Cyan Dashed Ring (Matching Screenshot) */}
        <span className="absolute inset-[3px] rounded-full border-2 border-dashed border-emerald-400/70 animate-spin-slow pointer-events-none group-hover:border-cyan-300" />

        {/* Neon Emerald-Cyan Gradient WhatsApp Icon */}
        <svg
          className="w-7 h-7 relative z-10 transition-transform duration-300 group-hover:scale-110 drop-shadow-[0_0_12px_rgba(52,211,153,0.9)]"
          viewBox="0 0 24 24"
        >
          <defs>
            <linearGradient id="whatsappNeonGradient" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#34d399" />
              <stop offset="50%" stopColor="#2dd4bf" />
              <stop offset="100%" stopColor="#38bdf8" />
            </linearGradient>
          </defs>
          <path
            fill="url(#whatsappNeonGradient)"
            d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"
          />
        </svg>

        {/* Ambient Emerald Outer Glow Ring */}
        <span className="absolute -inset-1 rounded-full bg-emerald-400/20 blur-md pointer-events-none" />
      </a>
    </div>
  );
}


