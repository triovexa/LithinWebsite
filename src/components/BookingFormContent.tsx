...
export default function BookingFormContent({
  ...
  useEffect(() => {
    if (initialContainer && CONTAINER_SIZES.includes(initialContainer.toLowerCase().trim())) {
      setFormData(prev => ({ ...prev, container: initialContainer.toLowerCase().trim() }));
    }
  }, [initialContainer]);
  ...
  useEffect(() => {
    const handleSelectFeet = (e: Event) => {
      ...
      if (customEvent.detail) {
        const targetSize = customEvent.detail.toLowerCase().trim();
        if (CONTAINER_SIZES.includes(targetSize)) {
          setFormData(prev => ({ ...prev, container: targetSize }));
        }
      }
    };
